...
public sealed class BudgetService(IAppDbContext db, CategoryService categories)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (duplicate)
        {
            throw AppException.Conflict("A budget already exists for that category and month.");
        }
