...
public sealed class TransactionService(IAppDbContext db, CategoryService categories)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Type == TransactionType.Transfer)
        {
            ...
            {
                throw AppException.BadRequest("A transfer requires a destination account.");
            }

            if (target == request.AccountId)
            {
                throw AppException.BadRequest("A transfer must use two different accounts.");
            }
