...
public sealed class RecurringService(IAppDbContext db, CategoryService categories)
{
    ...
    private async Task ValidateAsync(Guid userId, RecurringRuleRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            throw AppException.BadRequest("Recurring transfers are not supported.");
        }
        ...
        {
            throw AppException.BadRequest("End date must not be before the start date.");
        }
